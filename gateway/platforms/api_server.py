...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _run_agent(
        ...
        # ── Phoenix: trace agent run for API server ──────────────────
        _phx_start = time.time()
        _phx_span = None
        try:
            from gateway.phoenix_helpers import _get_tracer as _phx_get
            _phx_t = _phx_get()
            if _phx_t is not None:
                _phx_span = _phx_t.start_span(
                    "gateway.agent.run",
                    source="api_server",
                    session_id=session_id or "unknown",
                    message_len=len(user_message) if user_message else 0,
                )
        except Exception:
            pass
        ...
        if _phx_span is not None:
            try:
                _duration = (time.time() - _phx_start) * 1000
                _phx_span.set_attribute("duration_ms", _duration)
                _phx_span.set_attribute("response_len", len(result.get("final_response", "")))
                _phx_span.set_status("ok")
            except Exception:
                pass
            finally:
                _phx_span._finish()
