...
# /exec constants — direct command execution (no LLM in the loop)
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            # ecoseek: direct command execution endpoint
            ("POST", "/v1/exec", self._handle_exec),
            ("POST", "/exec", self._handle_exec),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        ...
        # Add hermes-fast and hermes-reasoner bypass model aliases
        for _fast_alias in ("hermes-fast", "hermes-reasoner"):
            if _fast_alias not in {m["id"] for m in models}:
                models.append({
                    "id": _fast_alias,
                    "object": "model",
                    "created": now,
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # ecoseek: hermes-fast / hermes-reasoner bypass — proxy directly to upstream
        ...
        # Attach hermes_trace when opt-in and available (ecoseek fork feature)
        if _trace_obj is not None and isinstance(result, dict):
            _trace_data = getattr(_trace_obj, "to_dict", None)
            if _trace_data:
                response_data["hermes_trace"] = _trace_data()
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # Attach hermes_trace telemetry result
                    if hermes_trace is not None and isinstance(result, dict):
                        _trace_data = getattr(hermes_trace, "to_dict", None)
                        if _trace_data is not None:
                            result["hermes_trace"] = _trace_data()
                    return result, usage
